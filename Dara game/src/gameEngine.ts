export class Game {
    public gamePhase: string = "drop";
    public playerOneTokens: number = 0;
    public playerTwoTokens: number = 0;
    public playerOneName: string;
    public playerTwoName: string;
    public gameMode: string;
    public playerOneMoves: boolean;
    public gameBoard: string[][];

    constructor(playerOneMoves: boolean, playerOneName: string, playerTwoName: string, gameMode: string){
    ...
    drop(y: number, x: number) {
    
        if (this.gameBoard[y][x] != ""){
            return;
        }
        let currentToken: string = this.findCurrentToken();
    ...
    move(tokenY: number, tokenX: number, cellY: number, cellX: number) {

        let playersToken: string = this.gameBoard[tokenY][tokenX];

        this.gameBoard[tokenY][tokenX] = "";
        this.gameBoard[cellY][cellX] = playersToken;

        let tokensInARow: number = this.threeOrFourInARow(cellY, cellX);
    ...
    threeOrFourInARow(y: number, x: number){
        let token: string = this.findCurrentToken();
    ...
    removeEnemyToken(y: number, x: number){
        this.gameBoard[y][x] = "";
    ...
    isLegalMove(oldY: number, oldX: number, newY: number, newX: number) {
    ...
    computerMove(){
        ...
        let randomNumber = Math.floor(Math.random() * possibleCellArr.length);
        let tokenCoord: number[] = possibleCellArr[randomNumber][0];
        let cellCoord: number[] = possibleCellArr[randomNumber][1];
    ...
    checkNaberCell(oldY: number, oldX: number, newY: number, newX: number){
        let possibleCellCoord;
        let cellCoord;
        let token = this.findCurrentToken();

        this.gameBoard[oldY][oldX] = "";
        this.gameBoard[newY][newX] = token;
        let tokensInRow = this.threeOrFourInARow(newY, newX);
        if (tokensInRow == 3){
            cellCoord = [newY, newX];
        } else {
            if (tokensInRow < 3){
